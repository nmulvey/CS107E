...
// Which branch instruction is used? beq 
...
// by using seqz and addi
int conditional(int arg) {
    if (arg != 0) { // change comparison from 0 to 13, what changes? more li 
        return 1;
    } else {
        return 2;
    }
...
// Which register is used for variable `i`? for variable `result`? a5 for i and a0 for result 
// Where are the instructions for loop init? loop test? loop increment?
// loop init is li a5,0 loop test is blt a5, a1, .L3, loop incrament is addiw a5, a5, 1
//
// How is control flow routed?
// it goes from loop to L2 then does the i < n then to L3 
// Compare the generated assembly at optimization level -Og with
// more aggressively optimized -O2.  What has changed? it became shorter
...
// qualified as `volatile` and why? I believe it is PB Dat
// tells compiler to not to optimize 
