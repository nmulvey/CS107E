...
void wait_for_click(void) {
    ...
    while (gpio_read(BUTTON) == 0) {
        // Busy wait until the button is released
    }
    
    // Now wait for the button to be pressed (falling edge)
    while (gpio_read(BUTTON) != 0) {
        // Busy wait until the button is pressed
        gCount++;
...
void config_button(void) {
    ...
    gpio_interrupt_config(pin, 1, TRUE);
